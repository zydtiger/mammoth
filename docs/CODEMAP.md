...
| `mammoth.workflow` | Schema-v1 `WorkflowConfig`, `RunConfig`, and `StepConfig`; caller-built `ProgrammaticWorkflow`, `ProgrammaticRun`, `ExecutionInputs` with optional intended phases and resume-checkpoint SHA-256, `ExecutionInputResolutionContext` and the typed post-lease execution-input resolver, `DispatchEntry`, `LifecycleEventContext`, typed lifecycle-field and dynamic child-environment providers, and read-only pre-dispatch context; `CommandPlan`, supervised process values, structured run/step/dispatch results, schema compilation, programmatic planning/validation/execution, YAML compatibility functions, and command construction |
...
caller-owned configuration
    └── mammoth.workflow.ProgrammaticWorkflow
        └── validate_programmatic_workflow → run_programmatic_workflow
            └── prepared run + held lease → optional execution-input resolver
                └── validated immutable execution context
            └── Mammoth-owned lifecycle events enriched from frozen caller fields
            └── non-dry child launch adds validated dynamic caller variables
                before Mammoth restores its owned join environment
```
