...
| `mammoth.workflow` | Schema-v1 `WorkflowConfig`, `RunConfig`, and `StepConfig`; caller-built `ProgrammaticWorkflow`, `ProgrammaticRun`, `ExecutionInputs`, `DispatchEntry`, `LifecycleEventContext`, typed lifecycle-field provider, and read-only pre-dispatch context; `CommandPlan`, supervised process values, structured run/step/dispatch results, schema compilation, programmatic planning/validation/execution, YAML compatibility functions, and command construction |
