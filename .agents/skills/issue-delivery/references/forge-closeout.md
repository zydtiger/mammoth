...
- Include anticipated acceptance-criterion wording or body amendments, checkbox edits, issue closing or reopening, eligible milestone closure, and safe task cleanup in the delivery plan. At the ready-to-merge gate, an unambiguous instruction to merge the identified pull request authorizes the normal compound merge, linked-issue reconciliation, and cleanup envelope defined in the main workflow without requiring the user to enumerate each routine action.
- Treat each body, issue-state, and milestone-state mutation as a forge write. Perform only writes covered by the compound merge authorization or another explicit approval; otherwise remain read-only and report the exact pending actions.
- Apply `$issue-pr-body-contract` directly to every issue-body mutation. Treat the write as incomplete until that skill succeeds.
...
4. Prepare each exact intended marker-only body and expected close or reopen state internally. An unambiguous merge instruction at this gate authorizes those routine per-issue writes for the unchanged readiness snapshot; do not require the user to restate them separately. Record the strongest body-revision and merge-time linked-issue-snapshot preconditions supported by the adapter. When either conditional protection is unavailable, block merge unless approval explicitly accepts that base-race limitation and its required immediate re-read. Block merge when any other closeout authority is missing.
5. When a linked issue declares no acceptance criteria, record only its marker, body, and count reconciliation as not applicable. Still record and verify its expected post-merge state; the compound merge authorization covers a routine close or reopen write needed to reach that state.
...
5. Compare the evaluation snapshot's raw body with this issue's current source base. If content drifted before evaluation, reconfirm that scope and acceptance criteria are unchanged, adopt the evaluated body as the new source base, and rebuild the authorized intended marker update from it. Otherwise obtain fresh approval for the material change and restart. If the rebuilt intended source already equals the evaluated body, skip publication and continue at step 8.
6. Immediately before publishing, re-read the raw body, complete discussion, and body revision identifier when available. If any moved from the evaluation snapshot, restart at step 1 so scope and evidence are reevaluated. Pass the strongest supported body-update or revision precondition to `$issue-pr-body-contract`. When no conditional update exists, proceed only under the explicitly approved base-race limitation and only if this immediate re-read still exactly matches the evaluation snapshot; report that the adapter could not prevent a concurrent write rather than claiming overwrite protection.
7. Apply the closeout workflow from `$issue-pr-body-contract` to the authorized base and intended bodies, then publish only satisfied acceptance-criterion markers. Continue only on `verified`; restart at step 1 on `target-moved`, and keep the issue open while resolving `source-invalid` or `publication-incomplete` through that skill.
8. After `verified`, re-read the issue body and complete discussion and confirm that nothing else moved. If either changed after contract verification, restart at step 1 for fresh scope processing, evidence evaluation, and authorization confirmation.
...
- an issue whose unrelated body content changes between merge approval and closeout, proving that no stale body is written and the original compound authority is retained only when scope and criteria are unchanged and the body contract validates the authorized marker update from the fresh body;
- an issue that changes after the immediate pre-write comparison, proving that the body contract returns `target-moved` and delivery restarts from a fresh scope, evidence, and authorization snapshot;
...
- a criterion changed after a `target-moved` result, proving that delivery restarts scope processing and evidence evaluation and obtains fresh approval before preparing another intended update;
...
- an issue with no milestone;
- a closeout without approval for forge mutations;
- a satisfied issue and clean task worktree completed from one unambiguous merge instruction, including marker reconciliation, issue closure, and safe cleanup without a second approval prompt;
- an imperative merge instruction received before the readiness snapshot, proving that it is not consumed and gate-time authorization is obtained after readiness is established;
- a hypothetical merge question or readiness request that remains read-only; and
- a failed or incomplete closeout that retains the worktree and branches instead of consuming cleanup authority.
